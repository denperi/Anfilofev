...
def begin_processes(vac_name, year, queue):
    ...
    data = [salary_year, vacancy_year, vacancy_salary, vac_count]
    queue.put(data)
...
if __name__ == "__main__":
    ...
    for process in processes:
        dicts = queue.get()
        sals_year.update(dicts[0])
        vacs_year.update(dicts[1])
        vacancy_salary.update(dicts[2])
        vac_count.update(dicts[3])
        process.join()
